...
def class_text_to_int(row_label):
    return label_id_map.get(row_label, 0)
    ...
    for data in data_list:
        ...
        try:
            for member in root.findall('object'):
                xmin = int(float(member.find('.//xmin').text))
                ymin = int(float(member.find('.//ymin').text))
                xmax = int(float(member.find('.//xmax').text))
                ymax = int(float(member.find('.//ymax').text))
                value = (data['image'],
                        width,
                        height,
                        member[0].text,
            ...
            print(f'----------------- Invalid label file:{data["image"]} ----------, skipped error:{e}')
            continue
...
if __name__ == '__main__':
    ...
    with open(os.path.join(annotations_dir, 'label_map.pbtxt'), 'w') as f:
        label_items = list(label_id_map.items())
        sorted(label_items, key=lambda item: item[1], reverse=True)
